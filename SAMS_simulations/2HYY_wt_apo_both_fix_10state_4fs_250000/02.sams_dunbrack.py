...
experiment = 'wt_apo_both_fix_10state_4fs' # setting of the experiment (e.g. different combinations of the CVs)
...
ndistances = 4 # number of distances we want to restrain
ntargets = 10 # number of sams states to bias to
targets = list(range(10)) # list of dunbrack clusters (sams states) to bias to
...
# Include D3, D4
df_d3d4 = pd.read_csv('./d3d4_data_clean.csv')
df_struct_d3d4 = pd.read_csv('./all_feature_rep_struct_data.csv')

new_mean = df_d3d4.groupby('Cluster').aggregate({'dis2_c' : 'mean', 'dis3_c' : 'mean'})
new_std = df_d3d4.groupby('Cluster').aggregate({'dis2_c' : 'std', 'dis3_c' : 'std'})

data_mean['D3'] = new_mean[['dis2_c']]
data_mean['D4'] = new_mean[['dis3_c']]
data_std['D3'] = new_std[['dis2_c']]
data_std['D4'] = new_std[['dis3_c']]
print(data_mean)
print(data_std)

# data contained in this dict: dunbrack (8) + structural (7) states
...
for i in range(8): # each of the 8 clusters
    ...
    for j in range(7, 11): # each of the 4 distances (nm)
        dunbrack_mean[i, j] = data_mean.loc[names[i], feature_names[j]]
        dunbrack_std[i, j] = data_std.loc[names[i], feature_names[j]]

for i in range(8, ntargets): # each of the 2 Structural clusters
    for j in range(7): # convert each of the 7 dihedrals to radians
        dunbrack_mean[i, j] = float(df_struct_d3d4.loc[i - 8, feature_names[j]]) / 180 * np.pi
        dunbrack_std[i, j] = data_std.mean(axis = 0)[j] # get the mean of the Dunbrack std
    for j in range(7, 11): # each of the 2 distances (nm)
        dunbrack_mean[i, j] = df_struct_d3d4.loc[i - 8, feature_names[j]]
        dunbrack_std[i, j] = data_std.mean(axis = 0)[j] # get the mean of the Dunbrack std
print(dunbrack_mean)
print(dunbrack_std)
