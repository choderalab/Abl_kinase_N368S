...
def compute_simple_protein_features(pdbid, chainid, coordfile, numbering):
    ...
    if coordfile == 'dcd':
        ...
        pdb = os.path.join(f'{pdbid}_chain{chainid}_apo_minequi.pdb')
    ...
    #       * four ditances (a 4*2 array where each row contains indices of the two atoms for each dihedral)
    dih = np.zeros(shape=(7, 4), dtype=int, order='C')
    dis = np.zeros(shape=(4, 2), dtype=int, order='C')
    ...
    if topology.select(f"chainid {chain_index} and residue {numbering[27]} and name CA"):
        ...
        dis[2][0] = 0
    if topology.select(f"chainid {chain_index} and residue {numbering[80]} and name CG"):
        dis[2][1] = topology.select(f"chainid {chain_index} and residue {numbering[80]} and name CG")
    else:
        dis[2][1] = 0
    if topology.select(f"chainid {chain_index} and residue {numbering[16]} and name CA"):
